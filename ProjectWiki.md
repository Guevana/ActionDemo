...
状态口径：`C++ 首版` 表示核心代码已落地并可编译；`待验证` 表示仍需编辑器场景跑通；`资产待配` 表示 C++ 入口存在但蓝图/资源尚未完成；`已验收` 只用于实际跑通过对应场景的链路。快速试错期间不记录临时代码，等编译通过且方向确定后再更新数据库条目。

复杂链路维护：表格行只保留可查询摘要；当单条逻辑链需要承载多阶段流程、边界条件或验收步骤时，可在对应数据库下追加同 ID 小节，避免把所有细节塞进一行导致后续维护遗漏。

文档状态：2026-05-30 更新。已核对 `ActionDemo.uproject`、`ActionDemo.Build.cs`、`Source/ActionDemo` 核心源码、`Content/ActionDemo` 关键资产引用；伤害/受击/死亡链路、角色插槽驱动命中窗口、武器插槽来源命中窗口、受击硬直/反馈链路、死亡表现清理链路已通过本机 UE 5.7 编译和编辑器场景验收；武器管理、生成、切换 C++ 已编译通过；`PLAN-Movement-01` 锁敌移动链路整体已验收；`PLAN-Attribute-01` 属性底座、GE 伤害计算入口、属性配置入口和攻击动作 `ActionData` 主数据迁移均已验收；`PLAN-Editor-01` 实时数据面板 Attribute/Tag/Ability 三栏 C++ 首版已编译，待编辑器场景验证。
...
| 本机引擎路径 | 当前机器默认 `E:\UE_5.7`，编译入口 `E:\UE_5.7\Engine\Build\BatchFiles\Build.bat`；若路径不存在，先探测本机 UE 5.7 安装或询问，不硬编码假路径 |
...
| MOD-Ability-Attack | `AbilitySystem/Abilities/ADGameplayAbility_AttackBase.*` | 攻击 Ability 只保留 GAS 身份、激活生命周期和 `ActionData` 引用；Montage、默认时长、伤害 GE、动作伤害系数和 Motion Warping 参数均从 `UADCombatActionData` 读取；缺少 `ActionData` 时攻击直接结束并记录 warning；死亡/受击硬直状态下阻止攻击激活 | 已验收 |
...
| MOD-Effect-Damage | `AbilitySystem/Effects/ADGameplayEffect_Damage.*`、`AbilitySystem/Effects/ADDamageExecutionCalculation.*` | 默认即时伤害 GE；执行计算捕获来源 `AttackPower`、目标 `Defense`，并读取动作 `Data.DamageScale` 修改 `Health`；不再读取 `Data.Damage`/基础伤害回退 | C++ 首版已编译，待编辑器验证 |
| MOD-Ability-TargetLock | `AbilitySystem/Abilities/ADGA_Target_Lock.*` | 锁敌 Ability；目标事实委托给 TargetingComponent | 已实现 |
| MOD-Data-CombatAction | `AbilitySystem/Data/ADCombatActionData.*` | 攻击动作主数据源；配置 Montage、默认时长、自动播放/结束、取消淡出、空中允许标记、伤害 GE、动作伤害系数 `DamageScale` 和 Motion Warping Target 参数；连段跳转继续由 StateTree 配置 | 已验收 |
...
| MOD-LockOnControl | `Components/Target/ADLockOnControlComponent.*` | 消费 TargetingComponent 的 `CurrentTarget`；锁定时缓存并覆盖移动旋转配置，按 Yaw 插值让玩家面向目标；锁定镜头按目标焦点平滑插值更新 Controller ControlRotation；锁定/解锁时切换 PlayerController 的 Free/LockOn 移动模式并传入目标和近距离参数；解锁、目标无效或角色死亡时关闭 Tick 并恢复原配置 | 已验收 |
...
| LC-Combo | 轻攻击连段如何跳转 | `Input.Attack.Light` 进入输入缓冲；StateTree 使用 `BufferedInputMatchesTag`、`CombatCurrentAction`、`LastCompletedActionMatches` 判断状态并选择下一段 Ability；`UADStateTreeTask_PlayCombatAbility` 消费输入、激活 Ability、记录 `TrackedActionTag/Serial`；旧动作被新实例替换时当前任务成功退出；`ActionData` 只提供当前动作参数，不负责解析连段跳转 | 已实现 |
| LC-AttackAbility | 攻击 Ability 生命周期 | `UADGameplayAbility_AttackBase` 要求配置 `ActionData`；Commit 后登记动作上下文；按 `ActionData` 决定是否播放 Montage、是否用默认时长兜底结束、取消旧 Montage 淡出时间；Montage 完成正常结束，中断/取消则取消结束；激活新攻击前 ASC 取消旧攻击 Ability | 已验收 |
...
| LC-HitDetection | 命中确认如何产生 | Montage `AD Hit Detection` 只给时机、插槽来源、双插槽和盒体截面配置；`UADHitDetectionComponent` 按 `SocketSource` 从 Montage 角色 Mesh 或 `UADWeaponManagerComponent` 当前装备武器的 Weapon Mesh 读取 `StartSocketName/EndSocketName`，生成世界空间盒体 Sweep、去重并构造 `FADCombatHitEventData`；`UADCombatComponent::HandleHitConfirmed()` 从当前攻击 `ActionData` 补全 `DamageEffectClass/DamageScale`、广播、发送 GAS GameplayEvent，并调用 Hit Receiver 接口；命中检测本身仍不直接扣血 | 已验收 |
| LC-Damage-ReceiveHit | 命中如何转为扣血和死亡 | 当前攻击 `ActionData` 提供 `DamageEffectClass/DamageScale`；`Event.Hit.Confirm` 使用 `OptionalObject` 携带 GE 类，并通过事件数值传递 `DamageScale`；`UADGameplayAbility_ReceiveHit` 优先使用来源 ASC 创建伤害 Spec；`UADDamageExecutionCalculation` 捕获来源 `AttackPower` 与目标 `Defense`，按 `AttackPower * DamageScale - Defense` 修改 `Health`，不再读取 `DamageAmount/Data.Damage` 作为基础伤害；敌人默认授予受击 Ability；`UADAttributeSet` Clamp 属性；`AADCharacterBase` 在 Health 归零时加 `State.Dead` 并清理运行态 | 已验收 |
...
| LC-LockOnWarpTarget | 攻击如何写入锁定目标 Motion Warping Target | `AADCharacterBase` 统一挂载 `UMotionWarpingComponent`；`UADGameplayAbility_AttackBase` 在 Commit 成功后按 `ActionData` 的 Motion Warping 配置读取 `UADTargetingComponent::CurrentTarget`，有效目标写入指定 Warp Target Transform，无锁定/目标死亡/目标无效时移除同名 Warp Target；Warp Transform 位置来自目标位置加配置偏移，旋转只保留攻击者指向目标的 Yaw；Combo1-4 Montage 均已接入 `MotionWarping` Notify State 并消费 `AttackTarget`，具体窗口以各 Montage 资产配置为准 | 已验收 |
...
| `State.Hit.React` | `State_Hit_React` | 受击硬直状态 |
...
| `Data.DamageScale` | `Data_DamageScale` | 动作伤害系数 SetByCaller |
...
| 攻击参数 | 攻击 Ability 只配置 `ActionData`；`UADCombatActionData` 配置 `AttackMontage`、`DefaultAttackDuration`、`bAutoPlayMontage`、`bAutoEndAbility`、`CancelMontageBlendOutTime`、`DamageEffectClass`、`DamageScale` 和 Motion Warping 参数；玩家轻攻击 1-3 已迁移到 `DA_ActionData_LightAttack_1/2/3` | 已验收 |
...
| 默认伤害 GE | `UADGameplayEffect_Damage` 为 instant GE；执行计算使用 `Source.AttackPower * Data.DamageScale - Target.Defense`，来源攻击力和动作系数大于 0 时最终至少 1 点伤害，不再需要 `Data.Damage` SetByCaller | 已验收 |
...
| 连段跳转 | 轻攻击连段跳转由玩家 Combat StateTree 配置；`UADCombatActionData` 只承载当前动作参数，不配置下一段 Ability | 已确认 |
...
| PLAN-Attribute-01 | 属性集与 GE 流水扩展 | 已验收：新增 AttackPower/Defense/Toughness/Energy/MaxEnergy，默认伤害 GE 完全由 `AttackPower` 与动作 `DamageScale` 接管并扣除目标 `Defense`；玩家从 `InputConfig`、敌人从 `EnemyConfig` 应用 `StartupAttributeEffects`；攻击动作配置已迁移到 `ActionData` 主数据源，玩家轻攻击 1-3 已完成资产配置和场景验收 |  `MOD-Attributes`、`MOD-Effect-Damage`、`MOD-Input`、`MOD-Character-Enemy`、`MOD-Ability-ReceiveHit`、`LC-Damage-ReceiveHit`、`PLAN-Combat-01` |
...
| 2026-05-19 | Planning Database 扩展为带阶段方案的下一步规划集合：受击链路、Motion Warping、属性扩展、弹反/极限闪避、空中连段、阶段验收 | `Planning Database` |
...
| 2026-05-28 | 锁定朝向控制已通过编辑器场景验收：锁定后玩家稳定面向目标，解锁或目标失效后恢复自由移动旋转配置 | `PLAN-Movement-01`、`LC-LockOnFacing`、`MOD-LockOnControl` |
| 2026-05-28 | 锁定镜头已通过编译和编辑器场景验收：锁定时平滑驱动 Controller ControlRotation 看向目标焦点，解锁后停止辅助但不回弹镜头 | `PLAN-Movement-01`、`LC-LockOnCamera`、`MOD-LockOnControl` |
| 2026-05-28 | 锁定移动输入职责拆分：PlayerController 维护移动模式枚举并实现 Free/LockOn 输入解释，LockOnControlComponent 只负责按目标状态切换模式并传入参数 | `PLAN-Movement-01`、`LC-LockOnMovement`、`MOD-PlayerController`、`MOD-LockOnControl` |
| 2026-05-28 | 锁定移动输入已通过编辑器场景验收：Free/LockOn 移动模式切换正常，锁定时 W/S 靠近远离目标、A/D 沿目标切线横移，解锁或目标失效后恢复自由移动 | `PLAN-Movement-01`、`LC-LockOnMovement`、`MOD-PlayerController`、`MOD-LockOnControl` |
...
| 2026-05-30 | 动作伤害系数收口到 `UADCombatActionData::DamageScale`，通过命中事件写入 Damage GE Spec，由执行计算消费；命中检测仍不计算伤害 | `PLAN-Attribute-01`、`MOD-Data-CombatAction`、`MOD-Effect-Damage`、`LC-HitDetection` |
| 2026-05-30 | 攻击动作配置直接迁移到 `UADCombatActionData`，攻击 Ability 不再保留 Montage、时长、伤害 GE 或 Motion Warping 参数入口 | `MOD-Ability-Attack`、`MOD-Data-CombatAction`、`LC-AttackAbility` |
| 2026-05-30 | 攻击动作 `ActionData` 主数据迁移已通过编辑器场景验收，玩家轻攻击 1-3 已绑定对应动作数据资产 | `PLAN-Attribute-01`、`MOD-Ability-Attack`、`MOD-Data-CombatAction` |
| 2026-05-30 | 连段跳转继续由 StateTree 负责，`ActionData` 不作为下一段 Ability 决策源 | `LC-Combo`、`MOD-Data-CombatAction` |
...
| 2026-05-19 | 扩充 Planning Database：受击链路、Motion Warping、属性、弹反/闪避、空中连段、阶段验收 | `Planning Database`、`Decision Ledger` |
...
| 2026-05-28 | 锁定朝向控制编辑器验收通过 | `PLAN-Movement-01`、`LC-LockOnFacing`、`MOD-LockOnControl` |
| 2026-05-28 | 落地锁定镜头 C++ 首版 | `PLAN-Movement-01`、`LC-LockOnCamera`、`MOD-LockOnControl` |
| 2026-05-28 | 锁定镜头编译和编辑器验收通过 | `PLAN-Movement-01`、`LC-LockOnCamera`、`MOD-LockOnControl` |
| 2026-05-28 | 落地锁定移动输入 C++ 首版 | `PLAN-Movement-01`、`LC-LockOnMovement`、`MOD-PlayerController`、`MOD-LockOnControl` |
| 2026-05-28 | 锁定移动输入编辑器验收通过 | `PLAN-Movement-01`、`LC-LockOnMovement`、`MOD-PlayerController`、`MOD-LockOnControl` |
...
| 2026-05-30 | 属性配置入口编辑器验收通过，动作数据新增 `DamageScale` 并接入默认伤害执行计算 | `PLAN-Attribute-01`、`MOD-Data-CombatAction`、`MOD-Effect-Damage` |
| 2026-05-30 | 攻击动作配置直接迁移到 ActionData 主数据源 | `MOD-Ability-Attack`、`MOD-Data-CombatAction` |
| 2026-05-30 | 攻击动作 ActionData 主数据迁移编辑器验收通过 | `PLAN-Attribute-01`、`MOD-Ability-Attack`、`MOD-Data-CombatAction` |
| 2026-05-30 | 保留 StateTree 连段跳转职责，移除不符合架构的连段规划 | `Planning Database`、`LC-Combo` |
