...
文档状态：2026-05-24 更新。已核对 `ActionDemo.uproject`、`ActionDemo.Build.cs`、`Source/ActionDemo` 核心源码、`Content/ActionDemo` 关键资产引用；伤害/受击/死亡链路与角色插槽驱动命中窗口已通过本机 UE 5.7 编译和编辑器场景验收；武器管理组件与武器插槽来源 C++ 已编译通过，待编辑器资产验证。
...
| 已有重点 | 轻攻击连段、取消窗口、输入缓冲、锁敌、命中事件、C++ 伤害/受击/死亡首版、敌人近战 AI 首版 |
...
| MOD-Equipment-WeaponManager | `Components/Equipment/ADWeaponManagerComponent.*` | 角色武器事实源；维护当前装备武器、武器 owner 绑定/清理、装备变化广播，并提供装备武器和武器 Mesh 查询函数 | 首版，C++ 已编译 |
...
| 编译验收 | `ActionDemoEditor Win64 Development` 使用 `E:\UE_5.7\Engine\Build\BatchFiles\Build.bat` 编译；伤害/受击/死亡链路与角色插槽驱动命中窗口已完成编辑器场景验收；武器管理组件与武器插槽来源 C++ 已编译通过但待编辑器资产验证；UBT 提示 VS 2022 编译器非首选版本但未阻断 | 2026-05-24 已通过 |
